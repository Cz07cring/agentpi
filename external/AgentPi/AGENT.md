...
```
app/
  AgentPi/                    # App target (entry point, AppDelegate, ContentView)
  modules/
    AgentPiCore/              # Swift Package — all core logic lives here
      Sources/AgentPi/
        Configuration/         # AgentPiProvider, AgentPiConfiguration, environment keys
...
`AgentPiProvider` is the central service locator, injected via SwiftUI environment:
```swift
.environment(\.agentPi, provider)
```
`AgentPiModifier` also injects: `statsService`, `displaySettings`, `themeManager`, `runtimeTheme`.
...
- UserDefaults keys are namespaced under `com.ring.agentpi.` (see `AgentPiDefaults`)
...
```bash
# Build (via Xcode)
xcodebuild -workspace app/AgentPi.xcodeproj/project.xcworkspace -scheme AgentPi build

# Run tests
xcodebuild -workspace app/AgentPi.xcodeproj/project.xcworkspace -scheme AgentPi test
```
...
- Custom themes: `~/Library/Application Support/AgentPi/Themes/`
