...
Download the latest AgentPi release from your configured distribution channel. The app is code-signed and notarized by Apple.
...
AgentPi supports two display modes:
...
AgentPi reads Claude Code session data from:
...
AgentPi reads Codex session data from `~/.codex/`:
...
Place YAML theme files in `~/Library/Application Support/AgentPi/themes/`. Themes are hot-reloaded on save.
...
AgentPi runs entirely on your machine. It does not collect, transmit, or store any data externally. The app simply reads your local CLI session files to display their status.
