...
public final class DiffWebViewCoordinator: NSObject {
  ...
  private func callJavaScript<T: Encodable>(_ method: String, with input: T) {
    do {
      ...
      // Prefer structured argument passing to avoid any string/binary transcoding issues.
      if #available(macOS 12.0, *) {
        callAsyncJavaScript(method: method, with: jsonObject)
        return
      }
      ...
      // Decode base64 as UTF-8 bytes before JSON.parse.
      // Fallback keeps compatibility if TextDecoder is unavailable.
      let script = """
      (function() {
        try {
  ...
  private func callAsyncJavaScript(method: String, with jsonObject: Any) {
    webView?.callAsyncJavaScript(
      "window.pierreBridge.\(method)(input);",
      arguments: ["input": jsonObject],
      in: nil,
      in: .page
    ) { result in
      if case .failure(let error) = result {
        DiffLogger.error("DiffWebViewCoordinator: callAsyncJavaScript error: \(error)")
      }
    }
  }
