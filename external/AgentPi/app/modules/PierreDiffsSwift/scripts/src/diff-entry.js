...
/**
 * Attempts to repair common UTF-8 mojibake where UTF-8 bytes were interpreted as Latin-1.
 */
function maybeFixMojibake(value) {
  if (typeof value !== 'string' || value.length === 0) return value;

  const cjkCount = (value.match(/[\u3400-\u9FFF]/g) || []).length;
  const latinNoiseCount = (value.match(/[ÃÂÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞßà-ÿ]/g) || []).length;
  const controlCount = (value.match(/[\u0080-\u009F]/g) || []).length;

  // Heuristic: skip if it does not look like mojibake.
  if (latinNoiseCount < 4 && controlCount == 0) {
    return value;
  }
  ...
  try {
    if (typeof TextDecoder !== 'undefined') {
      ...
      const percentEncoded = Array.from(value, (char) =>
        '%' + (char.charCodeAt(0) & 0xff).toString(16).padStart(2, '0')
      ).join('');
      repaired = decodeURIComponent(percentEncoded);
    }
  } catch {
    return value;
  }

  const repairedCjkCount = (repaired.match(/[\u3400-\u9FFF]/g) || []).length;
  // Accept only if repair materially improves readability.
  if (repairedCjkCount > cjkCount) {
    return repaired;
  }

  if (controlCount > 0 && repaired !== value) {
    return repaired;
  }
  ...
  renderDiff(inputData) {
    try {
      ...
      const oldFileObj = {
        ...
        contents: maybeFixMojibake(oldFile.contents || ''),
        lang: oldLang,
      };

      const newFileObj = {
        name: newFile.name || 'new',
        contents: maybeFixMojibake(newFile.contents || ''),
        lang: newLang,
      };
