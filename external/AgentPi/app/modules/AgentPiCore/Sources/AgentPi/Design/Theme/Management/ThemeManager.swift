...
public final class ThemeManager {
  ...
  public init() {
    Self.migrateLegacyThemesDirectoryIfNeeded()

    // Load the correct built-in theme synchronously to avoid flash on launch
    let saved = UserDefaults.standard.string(forKey: AgentPiDefaults.selectedTheme) ?? "claude"
  ...
  nonisolated static func getThemeColors(for theme: AppTheme) -> ThemeColors {
    ...
    case .custom:
      let primary = UserDefaults.standard.string(forKey: AgentPiDefaults.customPrimaryHex) ?? "#7C3AED"
      let secondary = UserDefaults.standard.string(forKey: AgentPiDefaults.customSecondaryHex) ?? "#FFB000"
      let tertiary = UserDefaults.standard.string(forKey: AgentPiDefaults.customTertiaryHex) ?? "#64748B"
      return ThemeColors(
  ...
  private func saveCurrentThemeSelection(_ themeId: String) {
    UserDefaults.standard.set(themeId, forKey: AgentPiDefaults.selectedTheme)
  }
  ...
  public static func themesDirectory() -> URL {
    ...
    migrateLegacyThemesDirectoryIfNeeded()
    return appSupport.appendingPathComponent("AgentPi/Themes")
  }
  ...
  private func installBundledThemesIfNeeded() {
    ...
    let versionKey = AgentPiDefaults.installedBundledThemeVersion(for: "sentry")
  ...
  private func persistYAMLPalette(for cacheKey: String, runtime: RuntimeTheme) {
    ...
    if let colors = yamlColorCache[cacheKey] {
      defaults.set(colors.primary, forKey: AgentPiDefaults.yamlPrimaryHex)
      defaults.set(colors.secondary, forKey: AgentPiDefaults.yamlSecondaryHex)
      defaults.set(colors.tertiary, forKey: AgentPiDefaults.yamlTertiaryHex)
      return
    }

    // Fallback when serving from runtime-only cache.
    if let primary = Self.hexString(from: runtime.brandPrimary) {
      defaults.set(primary, forKey: AgentPiDefaults.yamlPrimaryHex)
    }
    if let secondary = Self.hexString(from: runtime.brandSecondary) {
      defaults.set(secondary, forKey: AgentPiDefaults.yamlSecondaryHex)
    }
    if let tertiary = Self.hexString(from: runtime.brandTertiary) {
      defaults.set(tertiary, forKey: AgentPiDefaults.yamlTertiaryHex)
    }
  }
  ...
  private static func migrateLegacyThemesDirectoryIfNeeded() {
    let fileManager = FileManager.default
    let appSupport = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
    let legacyDir = appSupport.appendingPathComponent("AgentHub/Themes")
    let targetDir = appSupport.appendingPathComponent("AgentPi/Themes")

    guard fileManager.fileExists(atPath: legacyDir.path) else { return }
    guard !fileManager.fileExists(atPath: targetDir.path) else { return }
    ...
    do {
      ...
      try fileManager.moveItem(at: legacyDir, to: targetDir)
    } catch {
      ...
      if let entries = try? fileManager.contentsOfDirectory(
        ...
        for entry in entries {
          let destination = targetDir.appendingPathComponent(entry.lastPathComponent)
