...
public actor SessionMetadataStore {
  ...
  /// Database is stored in ~/Library/Application Support/AgentPi/session_metadata.sqlite
  public init() throws {
    ...
    try Self.migrateAppSupportDirectoryIfNeeded(baseURL: appSupportURL)
    let agentPiDir = appSupportURL.appendingPathComponent("AgentPi", isDirectory: true)
    try FileManager.default.createDirectory(
      at: agentPiDir,
      withIntermediateDirectories: true
    )

    let dbPath = agentPiDir.appendingPathComponent("session_metadata.sqlite")
    dbQueue = try DatabaseQueue(path: dbPath.path)

    try migrator.migrate(dbQueue)
  }

  private static func migrateAppSupportDirectoryIfNeeded(baseURL: URL) throws {
    let fileManager = FileManager.default
    let legacyDir = baseURL.appendingPathComponent("AgentHub", isDirectory: true)
    let targetDir = baseURL.appendingPathComponent("AgentPi", isDirectory: true)
    ...
    do {
      ...
      for entry in entries {
        let destination = targetDir.appendingPathComponent(entry.lastPathComponent, isDirectory: false)
