...
public struct TerminalLauncher {
  ...
  /// - Parameters:
  ///   - command: The command name to search for (default: "pi")
  ///   - additionalPaths: Additional paths to search from configuration
  /// - Returns: The full path to the executable if found, nil otherwise
  public static func findCodexExecutable(
    ...
    // Priority 1: AgentPi local install / runtime bins
    let localCandidates = [
      "\(homeDir)/.pi/agent/bin/\(command)",
      "\(homeDir)/.agentpi/bin/\(command)",
      "\(homeDir)/.agentpi/local/\(command)",
      "\(homeDir)/.codex/local/\(command)"
    ]
    for candidate in localCandidates where fileManager.fileExists(atPath: candidate) {
      return candidate
    }
    ...
    for nvmPath in nvmPaths {
      let executablePath = "\(nvmPath)/\(command)"
