...
public actor GitWorktreeService {
  ...
  private static var gitWorktreeTimeout: TimeInterval {
    let defaults = UserDefaults.standard
    if defaults.object(forKey: AgentPiDefaults.worktreeTimeoutSeconds) == nil {
      return 30.0
    }
    let configured = defaults.double(forKey: AgentPiDefaults.worktreeTimeoutSeconds)
    return min(max(configured, 5.0), 300.0)
  }
  ...
  public func findGitRoot(at path: String) async throws -> String {
    ...
    let output: String
    do {
      output = try await runGitCommand(["rev-parse", "--show-toplevel"], at: path)
    } catch {
      throw WorktreeCreationError.notAGitRepository(path)
    }
    ...
    gitRootCache[root] = root
    return root
  }

  /// Validates whether a path belongs to a git repository and resolves the git root.
  /// - Parameter path: Repository path or any child directory inside a repository.
  /// - Returns: Validation result used by launcher and repository picker fail-fast checks.
  public func validateRepository(at path: String) async -> RepoValidationResult {
    do {
      let root = try await findGitRoot(at: path)
      return .valid(gitRootPath: root)
    } catch WorktreeCreationError.notAGitRepository {
      return .invalid(
        code: "GIT_NOT_REPO",
        message: "Selected path is not a git repository: \(path)"
      )
    } catch {
      return .invalid(
        code: "GIT_NOT_REPO",
  ...
  public func createWorktreeWithNewBranch(
    ...
    if let startPoint = startPoint {
      let resolvedStartPoint = await resolveStartPointRef(startPoint, at: gitRoot)
      args.append(resolvedStartPoint)
    }
  ...
  /// Resolves a potentially ambiguous start point to a fully-qualified ref when possible.
  /// This avoids git warnings/failures like "refname 'foo/bar' is ambiguous".
  private func resolveStartPointRef(_ startPoint: String, at gitRoot: String) async -> String {
    let trimmed = startPoint.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmed.isEmpty else { return "HEAD" }

    if trimmed == "HEAD" || trimmed.hasPrefix("refs/") {
      return trimmed
    }

    var candidates: [String] = ["refs/heads/\(trimmed)"]

    // `origin/main` should resolve to `refs/remotes/origin/main`.
    candidates.append("refs/remotes/\(trimmed)")

    // `main` may only exist as remote tracking branch.
    if !trimmed.contains("/") {
      candidates.append("refs/remotes/origin/\(trimmed)")
    }

    for candidate in candidates {
      if await refExists(candidate, at: gitRoot) {
        return candidate
      }
    }
  ...
  private func refExists(_ ref: String, at gitRoot: String) async -> Bool {
    do {
      _ = try await runGitCommand(["rev-parse", "--verify", "--quiet", ref], at: gitRoot)
      return true
    } catch {
      return false
    }
  ...
  public static func sanitizeBranchName(_ branch: String) -> String {
    ...
    // Convert to a stable ASCII slug so branch/path labels stay short and safe in CLIs.
    ...
    var slugScalars: [UnicodeScalar] = []
    var previousWasSeparator = false
    for scalar in lowered.unicodeScalars {
      let value = scalar.value
      let isAsciiLower = value >= 97 && value <= 122
      let isDigit = value >= 48 && value <= 57
      let isSafePunctuation = scalar == "-" || scalar == "_" || scalar == "."
      if isAsciiLower || isDigit || isSafePunctuation {
        slugScalars.append(scalar)
    ...
    var slug = String(String.UnicodeScalarView(slugScalars))
    ...
    if let first = slug.first, first == "-" || first == "." {
      slug = "session-\(slug)"
    }
  ...
  public static func worktreeDirectoryName(for branch: String, repoName: String) -> String {
    let repoSlug = String(sanitizeBranchName(repoName).prefix(24))
    let branchSlug = sanitizeBranchName(branch)
    let shortBranch = String(branchSlug.prefix(20))
    let hash = stableHashSuffix(branch)
    return "\(repoSlug)-\(shortBranch)-\(hash)"
  }

  private static func stableHashSuffix(_ input: String) -> String {
    var hash: UInt64 = 14_695_981_039_346_656_037
    for byte in input.utf8 {
      hash ^= UInt64(byte)
      hash = hash &* 1_099_511_628_211
    }
    return String(format: "%06x", Int(hash & 0x00ff_ffff))
  }
  ...
  private func ensureProcessTerminated(_ process: Process) {
