...
public actor CLISessionMonitorService {
  ...
  public func addRepository(_ path: String) async -> SelectedRepository? {
    guard let gitRoot = await GitWorktreeDetector.resolveGitRoot(for: path) else {
      AppLogger.git.warning("[MonitorService] Reject non-git path: \(path, privacy: .public)")
      return nil
    }
    ...
    guard !worktrees.isEmpty else {
      AppLogger.git.warning("[MonitorService] No worktrees detected for git repo: \(gitRoot, privacy: .public)")
      return nil
    }
  ...
  public func addRepositories(_ paths: [String]) async {
    // Resolve to git roots and drop invalid/non-git paths.
    var resolvedPaths: [String] = []
    resolvedPaths.reserveCapacity(paths.count)
    for path in paths {
      if let gitRoot = await GitWorktreeDetector.resolveGitRoot(for: path) {
        resolvedPaths.append(gitRoot)
      }
    }

    let uniqueResolvedPaths = Array(Set(resolvedPaths)).sorted()

    // Filter out already-added repos
    let newPaths = uniqueResolvedPaths.filter { path in
    ...
    for path in newPaths {
      ...
      guard !worktrees.isEmpty else {
        AppLogger.git.warning("[MonitorService] Skip repository with no detected worktrees: \(path, privacy: .public)")
        continue
      }
      let repository = SelectedRepository(
        path: path,
  ...
  private func detectWorktrees(at repoPath: String) async -> [WorktreeBranch] {
    guard let gitRoot = await GitWorktreeDetector.resolveGitRoot(for: repoPath) else {
      AppLogger.git.warning("[MonitorService] detectWorktrees called with non-git path: \(repoPath, privacy: .public)")
      return []
    }
    ...
    if worktrees.isEmpty {
      // Fall back to single main worktree only when git metadata exists.
