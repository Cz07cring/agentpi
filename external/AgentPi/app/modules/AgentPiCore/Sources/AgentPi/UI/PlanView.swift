...
public struct PlanView: View {
  ...
  @MainActor
  private func beginLoadAttempt() -> UInt64 {
    loadAttemptId &+= 1
    let attempt = loadAttemptId
    isLoading = true
    errorMessage = nil
    scheduleTimeout(for: attempt)
    return attempt
  }
  ...
  @MainActor
  private func scheduleTimeout(for attempt: UInt64) {
    ...
    Task {
      ...
      await MainActor.run {
        ...
        isLoading = false
        errorMessage = "Plan loading timed out after \(timeout)s. Please retry."
      }
    }
  }
