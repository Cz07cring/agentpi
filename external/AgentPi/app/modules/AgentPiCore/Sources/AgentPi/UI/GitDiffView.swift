...
public struct GitDiffView: View {
  ...
  @State private var loadAttemptId: UInt64 = 0
  @State private var fileLoadAttemptIds: [UUID: UInt64] = [:]
  ...
  private func loadChanges(for mode: DiffMode) async {
    let attempt = beginLoadChangesAttempt(for: mode)
    ...
    do {
      ...
      guard isCurrentLoadAttempt(attempt) else { return }
  ...
  private func loadFileDiff(for file: GitDiffFileEntry, mode: DiffMode? = nil) {
    ...
    let attempt = nextFileLoadAttempt(for: file.id)
    loadingStates[file.id] = true
    fileErrorMessages[file.id] = nil

    let currentMode = mode ?? diffMode
    scheduleFileLoadTimeout(for: file.id, attempt: attempt)
    ...
    Task {
      do {
        ...
        await MainActor.run {
          guard isCurrentFileLoadAttempt(for: file.id, attempt: attempt) else { return }
  ...
  @MainActor
  private func beginLoadChangesAttempt(for mode: DiffMode) -> UInt64 {
    loadAttemptId &+= 1
    let attempt = loadAttemptId
    isLoading = true
    errorMessage = nil
    diffState = .empty
    diffContents = [:]
    ...
    treeCommonPrefix = ""
    expandedPaths = []
    scheduleLoadChangesTimeout(for: attempt, mode: mode)
    return attempt
  }

  @MainActor
  private func isCurrentLoadAttempt(_ attempt: UInt64) -> Bool {
    loadAttemptId == attempt
  }

  @MainActor
  private func scheduleLoadChangesTimeout(for attempt: UInt64, mode: DiffMode) {
    let timeout = loadingTimeoutSeconds
    Task {
      try? await Task.sleep(for: .seconds(Double(timeout)))
      await MainActor.run {
        guard isCurrentLoadAttempt(attempt), isLoading else { return }
        isLoading = false
        errorMessage = "\(mode.loadingMessage) timed out after \(timeout)s. Please retry."
      }
    }
  }

  @MainActor
  private func nextFileLoadAttempt(for fileId: UUID) -> UInt64 {
    let next = (fileLoadAttemptIds[fileId] ?? 0) &+ 1
    fileLoadAttemptIds[fileId] = next
    return next
  }

  @MainActor
  private func isCurrentFileLoadAttempt(for fileId: UUID, attempt: UInt64) -> Bool {
    fileLoadAttemptIds[fileId] == attempt
  }

  @MainActor
  private func scheduleFileLoadTimeout(for fileId: UUID, attempt: UInt64) {
    let timeout = loadingTimeoutSeconds
    Task {
      try? await Task.sleep(for: .seconds(Double(timeout)))
      await MainActor.run {
        guard isCurrentFileLoadAttempt(for: fileId, attempt: attempt), loadingStates[fileId] == true else { return }
        loadingStates[fileId] = false
        fileErrorMessages[fileId] = "Loading diff timed out after \(timeout)s. Select the file to retry."
      }
    }
  }

  private var loadingTimeoutSeconds: Int {
    let configured = UserDefaults.standard.integer(forKey: AgentPiDefaults.maxLoadingWithoutFeedbackSeconds)
