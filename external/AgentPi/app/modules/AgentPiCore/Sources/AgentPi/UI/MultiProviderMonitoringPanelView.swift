...
//  AgentPi
//
//  Combines Claude + Codex + AgentPi monitored sessions into a single panel.
//
...
public enum HubFilterMode: Int, CaseIterable {
  ...
  var displayName: String {
    switch self {
    case .all: return L10n.t("hub.filter.all", "All")
    case .claude: return L10n.t("hub.filter.claude", "Claude")
    case .codex: return L10n.t("hub.filter.codex", "Codex")
    case .pi: return L10n.t("hub.filter.agentpi", "AgentPi")
    case .needsAction: return L10n.t("hub.filter.needs_action", "Needs Action")
    }
  }
}
...
struct HubFilterControl: View {
  ...
  var body: some View {
    HStack(spacing: 12) {
      ...
      filterTab(for: .pi, count: piCount)
      filterTab(for: .needsAction, count: needsActionCount)
    }
  }
...
enum ProviderMonitoringItem: Identifiable {
  ...
  var id: String {
    ...
    case .pending(let provider, _, let pending):
      return "pending-\(provider.stableKey)-\(pending.id.uuidString)"
    case .monitored(let provider, _, let session, _):
      return "\(provider.stableKey)-\(session.id)"
    }
  }
...
public struct MultiProviderMonitoringPanelView: View {
  ...
  @AppStorage(AgentPiDefaults.hubLayoutMode)
  ...
  private var loadingState: some View {
    VStack(spacing: 12) {
      ProgressView()
        .scaleEffect(0.8)
      Text(L10n.t("hub.restoring_sessions", "Restoring sessions..."))
  ...
  private var filteredEmptyState: some View {
    VStack(spacing: 12) {
      ...
      Text(
        L10n.f(
          "hub.no_filtered_sessions",
          "No %@ Sessions",
          filterMode.displayName
        )
      )
        .font(.headline)
        .foregroundColor(.secondary)

      Button(L10n.t("hub.show_all", "Show All")) {
        filterMode = .all
      }
  ...
  private func shouldShowItem(_ item: ProviderMonitoringItem) -> Bool {
    ...
    case .pi: return item.providerKind == .pi
    case .needsAction: return itemAttentionState(item).isNeedsAction
    }
  }
  ...
  private var effectivePrimarySessionId: String? {
    ...
    return filteredItems.sorted(by: prioritizedSort).first?.id
  }
  ...
  private var piItemCount: Int {
    allItems.filter { $0.providerKind == .pi }.count
  }

  private var needsActionItemCount: Int {
    allItems.filter { itemAttentionState($0).isNeedsAction }.count
  }
  ...
  private var pendingItems: [ProviderMonitoringItem] {
    ...
    let piPending = piViewModel.pendingHubSessions.map {
      ProviderMonitoringItem.pending(provider: .pi, viewModel: piViewModel, pending: $0)
    }
    return claudePending + codexPending + piPending
  }
  ...
  private var monitoredItems: [ProviderMonitoringItem] {
    ...
    let piItems = piViewModel.monitoredSessions.map {
      ProviderMonitoringItem.monitored(provider: .pi, viewModel: piViewModel, session: $0.session, state: $0.state)
    }
    return claudeItems + codexItems + piItems
  }
  ...
  private var groupedMonitoredSessions: [(modulePath: String, items: [ProviderMonitoringItem])] {
    ...
    return grouped.sorted { $0.key < $1.key }
      .map { (modulePath: $0.key, items: $0.value.sorted(by: prioritizedSort)) }
  }
  ...
  private var emptyStateViewModel: CLISessionsViewModel {
    ...
    case .needsAction, .all:
      ...
      if !piViewModel.selectedRepositories.isEmpty { return piViewModel }
      return claudeViewModel
    }
  }

  private func prioritizedSort(_ lhs: ProviderMonitoringItem, _ rhs: ProviderMonitoringItem) -> Bool {
  ...
  private func itemAttentionState(_ item: ProviderMonitoringItem) -> AttentionState {
    switch item {
    case .pending:
      return AttentionResolver.resolve(status: nil, isPending: true, recentActivityDescriptions: [])
