...
public enum CommandPaletteAction: Identifiable {
  ...
  var title: String {
    switch self {
    case .newSession: return L10n.t("cmd_palette.new_session.title", "New Session")
    case .switchToSession(_, let name, _, _): return name
    case .selectRepository(_, let name): return name
    case .openSettings: return L10n.t("cmd_palette.open_settings.title", "Open Settings")
    case .toggleSidebar: return L10n.t("cmd_palette.toggle_sidebar.title", "Toggle Sidebar")
    }
  }

  var subtitle: String? {
    switch self {
    case .newSession: return L10n.t("cmd_palette.new_session.subtitle", "Start a new Claude, Codex, or AgentPi session")
    case .switchToSession(_, _, let provider, let firstMessage):
      ...
      return L10n.f("cmd_palette.switch_session.subtitle", "Switch to %@ session", provider.rawValue)
    case .selectRepository(let path, _): return path
    case .openSettings: return L10n.t("cmd_palette.open_settings.subtitle", "Open application settings")
    case .toggleSidebar: return L10n.t("cmd_palette.toggle_sidebar.subtitle", "Show or hide the sidebar")
    }
  }
  ...
  var searchAliases: [String] {
    switch self {
    case .newSession:
      return ["launch", "start", "session", "new"]
    case .openSettings:
      return ["settings", "preferences", "config"]
    case .toggleSidebar:
      return ["sidebar", "panel", "toggle"]
    case .switchToSession, .selectRepository:
      return []
    }
  }

  func matchesQuickQuery(_ query: String) -> Bool {
    guard !query.isEmpty else { return true }
    let normalizedQuery = query.lowercased()
    if title.lowercased().contains(normalizedQuery) { return true }
    if subtitle?.lowercased().contains(normalizedQuery) == true { return true }
    return searchAliases.contains { $0.contains(normalizedQuery) || normalizedQuery.contains($0) }
  }
}
...
public struct CommandPaletteView: View {
  ...
  private var filteredQuickActions: [CommandPaletteAction] {
    let query = normalizedQuery.lowercased()
    return quickActions.filter { $0.matchesQuickQuery(query) }
  }

  private var displayedActions: [CommandPaletteAction] {
    filteredQuickActions + filteredSessionActions
  }
  ...
  public var body: some View {
    ZStack {
      ...
      VStack(spacing: 0) {
        HStack(spacing: 12) {
          ...
          TextField(L10n.t("cmd_palette.search.placeholder", "Find focused sessions..."), text: $searchQuery)
          ...
          Text(L10n.t("cmd_palette.key.esc", "esc"))
    ...
    .onChange(of: displayedActionIDs) { _, _ in
      if !normalizedQuery.isEmpty && !filteredSessionActions.isEmpty {
        selectedIndex = filteredQuickActions.count
      } else {
        clampSelectedIndex()
      }
    }
  ...
  private var emptyState: some View {
    VStack(spacing: 12) {
      ...
      Text(L10n.t("cmd_palette.empty.title", "No Focused Sessions"))
        .font(.headline)
        .foregroundColor(.secondary)

      Text(L10n.t("cmd_palette.empty.subtitle", "Start or focus a session to show it here"))
        .font(.caption)
