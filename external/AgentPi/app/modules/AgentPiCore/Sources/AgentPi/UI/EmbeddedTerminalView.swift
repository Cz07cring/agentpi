...
public class TerminalContainerView: NSView, ManagedLocalProcessTerminalViewDelegate {
  ...
  func configure(
    ...
    let isPiRuntime = isPiLikeConfiguration(cliConfiguration)
  ...
  private func startCLIProcess(
    ...
    let shellCommand = joinedArgs.isEmpty
      ...
      : "cd '\(escapedPath)' && \(preExecPrefix)exec '\(escapedCLIPath)' \(joinedArgs)"
  ...
  private func applyPiEmbeddedStabilityFlags(_ args: [String], initialPrompt: String?) -> [String] {
    // Embedded panes are narrow and currently trigger known pi TUI overflows with custom UI resources.
    // Keep desktop sessions stable by default.
    let stabilityFlags = ["--no-extensions", "--no-skills", "--no-themes"]
    var result = args
    let insertionIndex: Int
    if let initialPrompt, !initialPrompt.isEmpty, let promptIndex = result.lastIndex(of: initialPrompt) {
      insertionIndex = promptIndex
    } else {
      insertionIndex = result.count
    }
    var nextIndex = insertionIndex
    for flag in stabilityFlags where !result.contains(flag) {
      result.insert(flag, at: nextIndex)
      nextIndex += 1
    }
    return result
  }
