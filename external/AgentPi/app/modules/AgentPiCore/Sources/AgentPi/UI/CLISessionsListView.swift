...
public struct CLISessionsListView: View {
    ...
    .alert(
      ...
      if let error = viewModel.worktreeDeletionError {
        if error.isOrphaned, let parentRepoPath = error.parentRepoPath {
          ...
          Button(L10n.t("common.try_again", "Try Again")) {
            Task {
              let worktree = error.worktree
              viewModel.clearWorktreeDeletionError()
      ...
      if let error = viewModel.worktreeDeletionError {
        if error.isOrphaned {
          Text(
            L10n.f(
              "sessions_list.orphaned_worktree_detail",
              "This worktree is orphaned and no longer tracked by git.\n\nPrune & Delete will run git worktree prune and remove this directory.\n\nPath: %@",
              error.worktree.path
            )
          )
        } else {
          Text(
            L10n.f(
              "sessions_list.worktree_delete_failed_detail",
              "Could not delete worktree at:\n%@\n\nError: %@",
              error.worktree.path,
              error.message
            )
          )
  ...
  @AppStorage(AgentPiDefaults.selectedSessionsPanelSizeMode)
  ...
  private var statusHeader: some View {
    VStack(spacing: 8) {
      if case .repositoryValidationFailed(let message) = viewModel.loadingState {
        HStack(spacing: 8) {
      ...
      HStack {
        Text(
          L10n.f(
            ...
            viewModel.selectedRepositories.count,
        ...
        .help(
          L10n.f(
            "sessions_list.help.alert_sound_delay_format",
            "Alert sound delay: %d seconds",
            viewModel.approvalTimeoutSeconds
          )
        )
