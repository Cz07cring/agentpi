...
public final class CLISessionsViewModel {
  ...
  private var currentCLICommand: String {
    ...
    switch providerKind {
    case .claude:
      return defaults.string(forKey: AgentPiDefaults.claudeCommand) ?? cliConfiguration.command
    case .codex:
      return defaults.string(forKey: AgentPiDefaults.codexCommand) ?? cliConfiguration.command
    case .pi:
      return defaults.string(forKey: AgentPiDefaults.piCommand) ?? cliConfiguration.command
    }
  }
  ...
  private var persistenceKey: String {
    AgentPiDefaults.selectedRepositories + "." + providerDefaultsSuffix
  }
  private var expansionStateKey: String {
    AgentPiDefaults.keyPrefix + "sessions.expansionState." + providerDefaultsSuffix
  }
  ...
  private var resolvedCodexDataPath: String {
    if let codexDataPath, !codexDataPath.isEmpty {
      return codexDataPath
    }
    switch providerKind {
    case .codex:
      return AgentPiConfiguration.defaultCodexDataPath()
    case .pi:
      return AgentPiConfiguration.defaultPiDataPath()
    case .claude:
      return AgentPiConfiguration.defaultCodexDataPath()
    }
  }
  private var monitoredSessionsKey: String {
    AgentPiDefaults.monitoredSessionIds + "." + providerDefaultsSuffix
  }
  private var terminalViewKey: String {
    AgentPiDefaults.sessionsWithTerminalView + "." + providerDefaultsSuffix
  }
  ...
  public func addRepository(at path: String) {
    ...
    Task {
      ...
      guard validation.isGitRepo, let gitRoot = validation.gitRootPath else {
        let message = validation.reasonMessage ?? "Selected path is not a git repository."
        loadingState = .repositoryValidationFailed(message: message)
        AppLogger.git.warning("[CLISessionsVM] Reject non-git repository path: \(path, privacy: .public)")
        return
      }

      // [CLISessionsVM] Calling monitorService.addRepository...")
      let added = await monitorService.addRepository(gitRoot)
      // [CLISessionsVM] monitorService.addRepository completed")
      if added == nil {
        loadingState = .repositoryValidationFailed(
          message: "Unable to add repository. Make sure the selected folder is a valid git repository."
        )
        AppLogger.git.warning("[CLISessionsVM] monitorService.addRepository returned nil for: \(gitRoot, privacy: .public)")
        return
      }
      loadingState = .idle
      error = nil
  ...
  private func watchForNewCodexSession(pending: PendingHubSession, worktree: WorktreeBranch) async {
    let codexPath = resolvedCodexDataPath
