...
### 批量任务运行器
通过可配置模板运行非交互式一次性命令。
实时 stdout/stderr 流式输出，智能 ANSI 转义清理，停止/重跑控制，耗时追踪，自动滚动到底部。
一键**停止**按钮发送 SIGTERM；20 秒看门狗定时器防止输出缓冲导致界面"假死"。
CI 安全执行（`CI=1`、`GIT_TERMINAL_PROMPT=0`），自动 `happy` 中继回退。
...
- **实时流式输出** — stdout 和 stderr 被捕获并实时显示在批量运行面板中，智能清理 ANSI 转义序列和终端控制字符
- **停止与取消** — 一键**停止**按钮向运行中的进程发送 SIGTERM；被取消的任务显示"已取消"状态（退出码 130），而非笼统的失败
- **看门狗定时器** — 若 20 秒内无可见输出，面板自动显示提示信息，避免界面看起来"假死"
- **自动滚动** — 输出面板在新数据到达时自动滚动到最新行
- **等待状态** — 当 CLI 输出为缓冲模式时（如 `claude -p` 非流式输出），面板显示实时已用时间计数器和数据块计数
- **进程生命周期** — 每个任务在启动后立即捕获 PID；重跑从同一模板和上下文重新启动
- **`happy` 自动检测** — 如果 `happy` 中继命令被用作批量模板，AgentPi 会自动回退到原生 CLI 进行本地执行
- **CI 安全环境** — 批量任务以 `CI=1`、`GIT_TERMINAL_PROMPT=0`、`GIT_ASKPASS=/usr/bin/false` 环境变量运行，防止交互式提示阻塞无头执行
